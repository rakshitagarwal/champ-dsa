...
- **Post Service:** validates `mediaIds` are owned + ready, inserts `post` row in sharded Postgres, emits `PostCreated` to [Kafka](/hld/message-queue).
...
- **Hydration:** feed cache stores ids+types only; `MGET post:{id}` from [Redis](/hld/caching-strategies) with fallback to Postgres replica; never store JPEGs in Redis.
...
2. **Hashtags:** `hashtag → postIds` in [Elasticsearch](/hld/nosql-databases) or a dedicated `hashtag_post` table (sharded by hashtag hash) with time-ordered ids; ingest parses `#tag` from caption on post creation.
