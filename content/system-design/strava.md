...
- **Segment geo index:** [Elasticsearch](/hld/nosql-databases) with `geo_shape` or S2 / H3 index on segment corridor; query by activity bounding box (+ buffer) to get ~50 candidates, not 5M.
- **Leaderboard store:** [Redis](/hld/caching-strategies) sorted set per segment (`score=elapsedMs`, member=`userId:activityId`) for overall + per-year shards; Postgres `effort` table is source of truth.
...
- **Redis leaderboard hot shard:** consistent hash shards `s:{segmentId}` by hash slot ([Redis](/hld/caching-strategies) cluster); top-page CDN cache absorbs 90% of reads.
...
1. **Live segments / beacons:** WebSocket per active activity with TTL in [Redis](/hld/caching-strategies); phone pushes location every 5s; server does corridor check live. Similar to [Uber](/hld/uber) location.
