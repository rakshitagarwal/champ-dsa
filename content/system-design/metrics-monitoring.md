...
- **Ingest Gateway:** validates schema, enforces cardinality limit (`max_series_per_metric`), hashes `(metric, tags)` to partition, writes to [Kafka](/hld/message-queue). Returns 429 if cardinality would explode.
- **[Kafka](/hld/message-queue) buffer:** decouples bursty ingest from TSDB write; retention 6h enough to absorb TSDB compaction pause or Writer crash.
...
- **Query Gateway/Engine:** parses PromQL-ish, fans out to TSDB shards by time range + metric hash, merges, applies `avg/p99` (p99 via pre-aggregated histograms/t-digests). Caches dashboard widget results in [Redis](/hld/caching-strategies) (30s) to absorb refresh storms.
...
**Data flow — query:** Dashboard `GET /query_range?http.latency last 1h step 1m` → Query Gateway checks [Redis](/hld/caching-strategies) widget cache → miss → fan-out to hot shard only (since 1h) → merge + `avg` per step → cache → return.
...
- **Caching:** Widget result cache in [Redis](/hld/caching-strategies) `key=hash(metric+tags+step+from+to)` TTL 30s absorbs dashboard refresh thundering herd. Query engine also caches seriesId → posting list for 1 min.
...
4. **Multi-tenancy:** Add `tenantId` label, enforce per-tenant cardinality and query isolation via [API Gateway](/hld/api-design) + row-level filter on TSDB.
