...
| Push fan-out | viral post 10M followers | 10M pushes × 2KB payload + token lookup | Needs chunked [Kafka](/hld/message-queue) partitions, not inline loop |
...
**Component roles:**
- **Notification API (ingest):** validates, checks idempotency (unique constraint), writes `notifications` row `queued`, publishes to [Kafka](/hld/message-queue) via transactional outbox, returns 202. Uses [Redis](/hld/caching-strategies) for dedupe window cache.
...
- **In-app inbox:** writes to feed store; badge count in [Redis](/hld/caching-strategies) `INCR/DECR` + periodic reconciliation to DB.
...
- **Digest:** Tumbling window per `(userId, groupingKey)` — buffer in [Redis](/hld/caching-strategies) or Kafka Streams state store; flush when `count ≥ threshold` or `window elapsed` → render single collapsed notification.
...
- **Caching:** Prefs + push tokens in [Redis](/hld/caching-strategies) with write-through; template renders cached by `(templateId, version, locale)` (immutable). Badge counts in Redis with nightly reconciliation job that `SELECT COUNT(*) WHERE is_read=false` and corrects drift.
