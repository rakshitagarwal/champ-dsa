...
Client (Mobile/Web)
 ...
 |        `--> publishes PostCreated → [Kafka](/hld/message-queue)
 |
 +-- Graph Service (follows, friends) → Postgres/Cassandra (adjacency)
 |
 +-- Feed Services  <--- Kafka consumers
 |     |-- Fan-out Service (hybrid push/pull)
 |     |-- Feed Cache ([Redis](/hld/caching-strategies) / [Cassandra](/hld/nosql-databases): userId → list<postId>)
 |     |-- Ranking Service (ML/heuristic)
 |     `--> Timeline Service (merge + paginate)
 |
 +-- Counter Service ([Redis](/hld/caching-strategies) + Cassandra) — likes, comments count
 |
...
**Component roles:**
- **Post Service:** validates, writes `posts` row to DB, uploads media to S3, publishes `PostCreated{ postId, authorId, timestamp }` to [Kafka](/hld/message-queue). Never fans out synchronously.
- **Graph Service:** owns `follows(followerId, followeeId)` adjacency; serves follower lists in pages. Cached in [Redis](/hld/caching-strategies).
...
- **Feed Cache:** per-user inbox: `userId → sorted list of postIds` (by score/time, capped at ~500-1000). Stored in [Redis](/hld/caching-strategies) (hot) + [Cassandra](/hld/nosql-databases) (durable, wide row). This is the **precomputed feed**.
...
- **Caching:** [Redis](/hld/caching-strategies) Cluster for inboxes (TTL + LRU), post hydrate cache (`post:{id} → JSON` 5m TTL). Celebrity outbox cached with replica.
...
2. **Counter service:** Likes via [Redis](/hld/caching-strategies) `INCR post:{id}:likes` + async flush to `posts.like_count` every second; don't `COUNT(*)` on hot posts.
