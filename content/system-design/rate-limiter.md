...
- **[Redis](/hld/caching-strategies) Cluster:** Holds windows/counters. Lua scripts run atomically per key. Replicated, with persistence off (recreatable) — cache, not DB. Cluster sharded by `hash(key)`.
- **Config Service:** Stores tiered rules (`free: 10/s, 100/min; paid: 100/s, 10K/min`), per-route overrides. Gateways cache rules with 5s TTL + pub/sub invalidation via [Redis](/hld/caching-strategies) or [Kafka](/hld/message-queue).
- **Audit / Analytics:** Async emit to [Kafka](/hld/message-queue) for dashboards and abuse detection — never blocks `allow()`.
...
Rate limit at the **first hop that can identify the principal** — usually the [API Gateway](/hld/api-design). Extra limits inside expensive services (LLM, search, payment) protect even if gateway is bypassed. Always return IETF `RateLimit-*` headers and `429 + Retry-After` (seconds or HTTP-date). Clients should respect it with exponential backoff. Log every deny to [Kafka](/hld/message-queue) for abuse dashboards. Do not 500, do not silently drop — make the contract explicit.
