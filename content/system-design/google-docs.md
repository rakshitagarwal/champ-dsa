...
| Presence | 50k docs × 3 viewers avg | 150k ephemeral entries | Fits in [Redis](/hld/caching-strategies) memory, TTL 30s |
...
- **Doc Server (the heart):** holds authoritative in-memory document (e.g., string + attributes). On `op(baseRev, ops)`, transforms against concurrent ops since `baseRev`, assigns `seq = ++revision`, appends to durable log ([Kafka](/hld/message-queue) or Postgres `doc_ops`), applies to memory, broadcasts `remoteOp(seq, ops)` to all subscribers. ACKs the sender.
...
- **Presence Service:** not durable — cursor updates via WS → in-memory → [Redis](/hld/caching-strategies) with TTL; broadcast via same WS fan-out; cleared on disconnect.
- **[Kafka](/hld/message-queue) / opLog:** durability + replay for history and snapshot rebuild if doc server crashes.
...
**Presence is not edits.** Cursors churn at 10Hz — don't write them to Postgres or Kafka. Keep them in-memory on the doc server plus [Redis](/hld/caching-strategies) `SETEX doc:{id}:presence:{userId} '{cursor:15,color:red}' 30` and pub/sub fan-out. On disconnect, TTL expires and user fades from UI.
...
- **Replication & failover:** Each doc primary has a warm standby (replica subscribes to same [Kafka](/hld/message-queue) partition). On primary crash, router promotes standby (takes ~1-2s), replays unapplied Kafka tail, clients auto-reconnect. S3 snapshot + Kafka log = no data loss if both die? Replay from log.
...
- **Replication:** Postgres primary-replica for metadata; doc ops in [Kafka](/hld/message-queue) RF=3 or Postgres with streaming replica. S3 is 11-nines durable.
- **Failure modes:**
...
4. **Search:** Index snapshots in [Elasticsearch](/hld/nosql-databases) asynchronously via Kafka consumer; search doesn't block editing.
